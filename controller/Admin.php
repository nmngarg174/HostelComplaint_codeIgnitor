...
// the finished password
    ...
    function sendmail() {
        ...
        foreach ($data as $row) {
        if($row['email']== 'wardenj@thapar.edu'|| $row['email']== 'imcool.saurabh@gmail.com') continue;
            echo $row['email']."<br>";
            
            $salt = "thispasswordcannotbehacked";
            $password = $this->generate_password();
            $pass = hash('sha256', $salt . $password);
            $this->Admin_model->insertLogin($row['email'], $pass);
            $to = $row['email'];
...
<br>
<pre>
<b>Name     :  </b>" . $row['fname'] . "<br>
<b>Username :  </b>" . $row['email'] . "<br>
<b>Password :  </b>" . $password . "
<br>
</pre>
http://onlinehostelj.in/
<br>
<b style='color:red'>Note : No complaint would be entertained through the caretaker office.<br>
